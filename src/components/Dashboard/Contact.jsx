...
const ContactSection = () => {
  ...
  const handleDelete = async (msg) => {
    const result = await Swal.fire({
      ...
      confirmButtonColor: '#ff4444', cancelButtonColor: '#444',
  ...
  const handleReply = async (msg) => {
    const { value: formValues } = await Swal.fire({
      ...
      confirmButtonText: 'إرسال الرد', cancelButtonText: 'إلغاء',
      confirmButtonColor: '#D4AF37', cancelButtonColor: '#444',
      background: '#1a1a1a', color: '#fff', focusConfirm: false,
      preConfirm: () => {
        const status  = document.getElementById('swal-status').value;
        const message = document.getElementById('swal-message').value;
        if (!message || !message.trim()) { Swal.showValidationMessage('لازم تكتب رد قبل الإرسال'); return false; }
        return { status, message };
      }
    });
    if (!formValues) return;

    Swal.fire({ title: 'جاري إرسال الرد...', allowOutsideClick: false, allowEscapeKey: false, background: '#1a1a1a', color: '#fff', didOpen: () => Swal.showLoading() });
    ...
    try {
      ...
      } catch (delErr) { console.error("Delete after reply failed:", delErr); }

      await Swal.fire({
        title: 'تم إرسال الرد بنجاح', text: `تم إرسال ردك إلى ${msg.email}`,
        icon: 'success', confirmButtonText: 'تم', confirmButtonColor: '#D4AF37',
      ...
      Swal.fire({
        ...
        icon: 'error', confirmButtonColor: '#D4AF37', background: '#1a1a1a', color: '#fff'
      });
    }
  };
      ...
      <div className="flex items-center gap-1 bg-white/5 border border-white/10 rounded-2xl p-1.5 w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-black transition-all ${
            activeTab === 'general' ? 'bg-[#D4AF37] text-black' : 'text-gray-400 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-black transition-all ${
            activeTab === 'store' ? 'bg-[#D4AF37] text-black' : 'text-gray-400 hover:text-white'
          }`}
        >
