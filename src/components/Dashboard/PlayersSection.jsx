...
// 🌟 استيراد مكتبة السويت اليرت
import Swal from 'sweetalert2';

const PlayersSection = ({ activeTab }) => {
  ...
  // 🌟 دالة تحديث حالة الاختبار باستخدام SweetAlert2 الاحترافي
  const handleMarkAsDone = async (player) => {
    Swal.fire({
      ...
      confirmButtonColor: '#10B981', // لون أخضر جذاب ومتناسق
      cancelButtonColor: '#ef4444', // لون أحمر للإلغاء
      background: '#1A1D1E', // خلفية داكنة متناسقة مع براند المنصة
      color: '#ffffff', // لون نصوص السويت اليرت أبيض لتكون واضحة
      customClass: {
        popup: 'rounded-[1.5rem] border border-white/10 shadow-2xl'
      }
    }).then(async (result) => {
      // لو الكابتن داس على زر التأكيد (نعم، تم اختباره)
      if (result.isConfirmed) {
        try {
          ...
          const res = await fetch(`${API_URL}/players/${player.id}`, {
            ...
            body: JSON.stringify({ fieldTest: updatedFieldTest }),
          });

          if (res.ok) {
            // تحديث الواجهة المحلية فوراً
            setPlayers(prev => prev.map(p => p.id === player.id ? { ...p, fieldTest: updatedFieldTest } : p));
            
            // تحويل الكابتن لصفحة التقييم
  ...
  // دالة لحساب الموقف الزمني للاختبار الفني
  const getCountdownStatus = (dateString) => {
    ...
    try {
      ...
      if (diffDays < 0) {
        return { text: "عدّى ميعاده ⚠️", className: "text-red-400 bg-red-500/10 border border-red-500/20" };
      } else if (diffDays === 0) {
        return { text: "اليوم ⚽", className: "text-emerald-400 bg-emerald-500/20 border border-emerald-500/30 animate-pulse font-black" };
      } else {
        return { text: `متبقي ${diffDays} يوم ⏱️`, className: "text-sky-400 bg-sky-500/10 border border-sky-500/20" };
      }
    } catch (e) {
      return null;
    }
  ...
  // الفلترة الأساسية بحسب التبويب النشط والبحث
  ...
  // تقسيم المجموعات للاعبين المقبولين مبدئياً
                  ...
                  <tr key={player.id} className="hover:bg-white/[0.02] transition-all group">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex flex-col gap-1 mt-1">
                          {player.fieldTest?.isDone ? (
                            player.fieldTest.finalStatus === 'accepted' ? (
                              <span className="text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-md flex items-center gap-1 font-bold w-fit"><CheckCircle className="w-3 h-3"/> مقبول نهائي</span>
                            ) : player.fieldTest.finalStatus === 'rejected' ? (
                              <span className="text-red-400 bg-red-500/10 px-2 py-0.5 rounded-md flex items-center gap-1 font-bold w-fit"><XCircle className="w-3 h-3"/> مستبعد ميدانياً</span>
                            ) : (
                              <span className="text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded-md flex items-center gap-1 font-bold w-fit animate-pulse"><Clock className="w-3 h-3"/> تم الاختبار (بانتظار القرار النهائي)</span>
                            )
                          ) : player.status === 'approved' ? (
                            hasSchedule ? (
                              <div className="space-y-1 bg-white/[0.02] p-2 rounded-xl border border-white/5 mt-1 max-w-[220px]">
                                <div className="flex items-center gap-1.5 justify-between">
                                  ...
                                  {countdown && (
                                    <span className={`px-1.5 py-0.5 rounded-md text-[10px] font-black ${countdown.className}`}>
                                      {countdown.text}
                                    </span>
                                  )}
                                </div>
                    ...
                    <td className="p-6">
                      <div className="flex justify-center items-center gap-2">
                        ...
                        {/* زر "تم الاختبار" الذي يشغل السويت اليرت الاحترافي ⚽ */}
                        {showActionType === 'mark_done' && (
                          <button 
                            onClick={() => handleMarkAsDone(player)}
                            className="px-3 py-2 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-xl text-xs font-black hover:bg-emerald-500 hover:text-black transition-all flex items-center gap-1 shadow-md"
                          >
                        ...
                        {showActionType !== 'go_evaluate' && (
                          <button 
                            onClick={() => navigate(`/dashboard/player/${player.id}`)}
                            className="px-3 py-2 bg-[#D4AF37]/10 text-[#D4AF37] border border-[#D4AF37]/20 rounded-xl text-xs font-black hover:bg-[#D4AF37] hover:text-black transition-all flex items-center gap-1.5 shadow-md"
                          >
  ...
  return (
    <div className="space-y-8 animate-fadeIn" dir="rtl">
      ...
      {activeTab === 'approved_waiting' ? (
        <div className="space-y-10">
          
          {/* البوكس الأول: تم اختباره وبانتظار وضع القرار النهائي */}
          <div className="space-y-3">
            ...
            <RenderPlayersTable 
              ...
              emptyMessage="لا يوجد لاعبين معلقين بانتظار التقييم النهائي حالياً." 
              showActionType="go_evaluate"
            />
          </div>

          {/* البوكس الثاني: مجدولين وبانتظار نزول الملعب */}
          <div className="space-y-3">
            <div className="flex items-center gap-2 px-2">
            ...
            <RenderPlayersTable 
              ...
              emptyMessage="لا يوجد لاعبين متاحين في جدول الترتيب الحالي." 
              showActionType="mark_done"
            />
          </div>

          {/* البوكس الثالث: مقبولين مبدئياً وبانتظار تحديد موعد */}
          <div className="space-y-3">
            <div className="flex items-center gap-2 px-2">
            ...
            <RenderPlayersTable 
              ...
              emptyMessage="لا يوجد لاعبين معلقين بانتظار تحديد موعد حالياً." 
              showActionType="none"
            />
          </div>
        ...
        <div className="space-y-3">
          ...
          <RenderPlayersTable 
            playersList={currentFilteredPlayers} 
            emptyMessage="لا يوجد لاعبين في هذا الكشف حالياً." 
            showActionType="none"
          />
        </div>
      )}
