...
const StatusBadge = ({
  ...
  return <span className={`inline-flex items-center gap-1 text-[10px] font-black px-2 py-1 rounded-lg border ${s.bg}`}>
      {s.icon} {s.label}
    </span>;
};
const CampRegistrationsSection = () => {
  ...
  useEffect(() => {
    const fetchRegistrations = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${API_URL}/camp-registrations`);
  ...
  const updateStatus = async (reg, newStatus) => {
    ...
    try {
      const res = await fetch(`${API_URL}/camp-registrations/${reg.id}`, {
        method: 'PATCH',
  ...
  return <div className="space-y-6 animate-fadeIn" dir="rtl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-[var(--bg-color-card)] dark:bg-white/5 p-4 rounded-[2rem] border border-white/10 shadow-2xl">
        ...
        border: 'dark:border-white/10 border-[var(--color-border)]'
      }, {
        key: 'pending',
        label: 'قيد المراجعة',
        color: 'text-yellow-400',
        border: 'dark:border-yellow-500/20 border-[var(--color-border)]'
      }, {
        key: 'accepted',
        label: 'مقبولين',
        color: 'text-emerald-400',
        border: 'dark:border-emerald-500/20 border-[var(--color-border)]'
      }, {
        key: 'rejected',
        label: 'مرفوضين',
        color: 'text-red-400',
        border: 'border-red-500/20 '
      }].map(s => <div key={s.key} className={`bg-white/5 rounded-2xl border ${s.border} p-4 text-center`}>
      ...
      <div className="flex flex-wrap gap-2 bg-white/5 p-1.5 rounded-2xl border border-[var(--color-border)] dark:border-white/5 w-fit">
        ...
        key: 'rejected',
        label: '❌ المرفوضين'
      }].map(f => <button key={f.key} onClick={() => setFilterStatus(f.key)} className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${filterStatus === f.key ? 'bg-[#D4AF37] text-black' : 'dark:text-gray-400 text-[var(--text-color-gray)] dark:hover:text-white'}`}>
            {f.label}
          </button>)}
      </div>
          ...
          <table className="w-full text-right">
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.length === 0 ? <tr>
                  <td colSpan="6" className="p-10 text-center text-sm dark:text-gray-500 font-bold">
                ...
                </tr> : filtered.map(reg => <tr key={reg.id} className="dark:hover:bg-white/[0.03] transition-all">
                    ...
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="font-bold text-sm dark:text-[var(--color-text-white)]">{reg.playerName}</p>
                      </div>
                    </td>
                    ...
                    <td className="p-5 text-xs dark:text-gray-400">
                      {reg.registeredAt ? new Date(reg.registeredAt).toLocaleDateString('ar-EG', {
                  year: 'numeric',
                  month: 'short',
                  day: 'numeric'
                }) : '—'}
                    ...
                    <td className="p-5">
                      <div className="flex items-center justify-center gap-2">
                        {reg.status !== 'accepted' && <button onClick={() => updateStatus(reg, 'accepted')} className="px-3 py-1.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-lg text-xs font-bold hover:bg-emerald-600 dark:hover:text-white hover:border-emerald-600 transition-all">
                            ✅ قبول
                          </button>}
                        {reg.status !== 'rejected' && <button onClick={() => updateStatus(reg, 'rejected')} className="px-3 py-1.5 bg-red-500/10 text-red-400 border border-red-500/20 rounded-lg text-xs font-bold hover:bg-red-600 dark:hover:text-white hover:border-red-600 transition-all">
                            ❌ رفض
                          </button>}
                        {reg.status === 'accepted' && reg.status !== 'rejected' && <span className="text-[10px] dark:text-gray-600 font-bold">تم القبول</span>}
                      </div>
                    </td>
