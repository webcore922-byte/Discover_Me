...
const CampsSection = () => {
  ...
  const [campForm, setCampForm] = useState({
    title: '', subtitle: '', location: '', schedule: '',
    details: '', image: '', focus: [], coachName: '', coachImage: ''
  });
  ...
  const handleSaveCamp = async (e) => {
    ...
    const payload = {
      ...
      focus:    typeof campForm.focus === 'string'
                  ? campForm.focus.split(',').map(f => f.trim())
                  : campForm.focus,
      coach: {
  ...
  return (
    <div className="space-y-8 animate-fadeIn text-[var(--color-text-gray)] " >
      ...
      <div className="flex gap-2 dark:bg-white/5 bg-[var(--color-bg-card)] p-1.5 rounded-2xl border border-white/5 w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black transition-all ${
            activeTab === 'manage' ? 'bg-sky-500 dark:text-white' : 'dark:text-gray-400 dark:hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black transition-all ${
            activeTab === 'registrations' ? 'bg-[#D4AF37] text-black' : 'dark:text-gray-400 dark:hover:text-white'
          }`}
        >
            ...
            <form onSubmit={handleSaveCamp} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="text" name="title" value={campForm.title} onChange={handleCampInputChange} className="w-full dark:bg-white/5 bg-[var(--color-bg-main)] border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-sky-500 dark:text-white" placeholder="مثال: معسكر حراسة" required />
                </div>
                <div>
                  <label className="text-xs dark:text-gray-400 block mb-1">العنوان الفرعي</label>
                  <input type="text" name="subtitle" value={campForm.subtitle} onChange={handleCampInputChange} className="w-full dark:bg-white/5 bg-[var(--color-bg-main)] border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-sky-500 dark:text-white" placeholder="مثال: معسكر تدريب حراس المرمى" required />
                </div>
                <div>
                  <label className="text-xs dark:text-gray-400 block mb-1">رابط صورة المعسكر</label>
                  <input type="url" name="image" value={campForm.image} onChange={handleCampInputChange} className="w-full dark:bg-white/5 bg-[var(--color-bg-main)] border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-sky-500 dark:text-white" placeholder="https://..." required />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="text" name="location" value={campForm.location} onChange={handleCampInputChange} className="w-full dark:bg-white/5 bg-[var(--color-bg-main)] border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-sky-500 dark:text-white" placeholder="مثال: القاهرة" required />
                </div>
                <div>
                  <label className="text-xs dark:text-gray-400 block mb-1">المواعيد والجدول</label>
                  <input type="text" name="schedule" value={campForm.schedule} onChange={handleCampInputChange} className="w-full dark:bg-white/5 bg-[var(--color-bg-main)] border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-sky-500 dark:text-white" placeholder="مثال: السبت والأحد: 10 صباحاً" required />
                </div>
                <div>
                  <label className="text-xs dark:text-gray-400 block mb-1">نقاط التركيز (افصل بفصلة ,)</label>
                  <input type="text" name="focus" value={campForm.focus} onChange={handleCampInputChange} className="w-full dark:bg-white/5 bg-[var(--color-bg-main)] border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-sky-500 dark:text-white" placeholder="التصدي, القيادة, السرعة" />
                </div>
              </div>
              ...
              <div className="p-4 bg-white/[0.02] border border-white/5 rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" name="coachName" value={campForm.coachName} onChange={handleCampInputChange} className="w-full dark:bg-white/5 bg-[var(--color-bg-main)] border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-sky-500 dark:text-white" placeholder="مثال: كابتن أحمد حسن" required />
                </div>
                <div>
                  <label className="text-xs text-sky-400 font-bold block mb-1 flex items-center gap-1">
                    <User className="w-3.5 h-3.5" /> رابط صورة المدرب
                  </label>
                  <input type="url" name="coachImage" value={campForm.coachImage} onChange={handleCampInputChange} className="w-full dark:bg-white/5 bg-[var(--color-bg-main)] border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-sky-500 dark:text-white" placeholder="https://..." />
                </div>
              </div>

              <div>
                <label className="text-xs dark:text-gray-400 block mb-1">تفاصيل ومميزات المعسكر</label>
                <textarea name="details" rows="3" value={campForm.details} onChange={handleCampInputChange} className="w-full dark:bg-white/5 bg-[var(--color-bg-main)] border border-white/10 rounded-xl px-4 py-2 text-xs outline-none focus:border-sky-500 dark:text-white" placeholder="اكتب تفاصيل المعسكر هنا..." required />
              </div>
                      ...
                      <tr key={item.id} className="hover:bg-white/5 transition-all ">
                        <td className="p-6">
                          <div className="flex items-center gap-4">
                            <img src={item.image} className="w-16 h-12 rounded-xl object-cover border border-[var(--color-border)] dark:border-white/10 shrink-0" alt="" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1508098682722-e99c43a406b2?q=80&w=500'; }} />
                            <div>
                              <p className="font-bold  dark:text-white">{item.title}</p>
                              <p className="text-xs dark:text-gray-400 mb-1">{item.subtitle}</p>
                              {item.coach?.name && (
                                <div className="flex items-center gap-1.5 mt-1 bg-white/5 w-fit px-2 py-0.5 rounded-md border border-[var(--color-border)] dark:border-white/10">
                        ...
                        <td className="p-6 text-xs dark:text-gray-400 max-w-[220px]">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {Array.isArray(item.focus) ? item.focus.map((f, i) => (
                              <span key={i} className="bg-sky-500/10 text-sky-400 text-[9px] px-1.5 py-0.5 rounded">{f}</span>
                            )) : null}
                          </div>
                        </td>
                        <td className="p-6">
