...
const FinalAccept = () => {
  ...
  const handleDecision = async decision => {
    ...
    const result = await Swal.fire({
      title: 'هل أنت متأكد؟',
      text: decision === 'accepted' ? `هل تود اعتماد "القبول النهائي" للاعب ${player.name} وإرسال بريد إلكتروني له؟` : `هل تود اعتماد "الرفض النهائي" للاعب ${player.name} وإرسال بريد إلكتروني له؟`,
      icon: 'question',
          ...
          <div className="lg:col-span-5">
            <div className="bg-[var(--color-bg-card)] rounded-[2.5rem] p-8 border border-[var(--color-border)]/20 h-full flex flex-col justify-between space-y-8 shadow-[var(--card-shadow)]">
              <div>
                ...
                <button onClick={handleOpenVideo} className="w-full py-5 bg-[var(--color-bg-main)]/30 border border-[var(--color-border)]/30 rounded-2xl text-[var(--color-gold-main)] font-black hover:bg-[var(--color-gold-main)] hover:text-black transition-all flex items-center justify-center gap-3">
              ...
              <div className="bg-[var(--color-bg-main)]/20 p-6 rounded-3xl border border-[var(--color-border)]/20 space-y-4">
                ...
                {playerStatus === 'final_accepted' || playerStatus === 'final_rejected' ? <div className="text-center py-2 flex flex-col gap-2 items-center">
                    ...
                    <button onClick={() => setPlayerStatus('')} className="text-[11px] text-[var(--color-gold-main)] underline hover:opacity-80 transition-colors mt-1">
                      تعديل القرار الحالي
                    </button>
                  </div> : <div className="flex gap-4 pt-2">
                    <button onClick={() => handleDecision('accepted')} className="flex-1 py-4 bg-green-600 hover:bg-green-700 text-white font-black rounded-2xl text-sm transition-colors uppercase italic tracking-wider shadow-lg shadow-green-900/20">
                      قبول نهائي
                    </button>
                    <button onClick={() => handleDecision('rejected')} className="flex-1 py-4 bg-red-600 hover:bg-red-700 text-white font-black rounded-2xl text-sm transition-colors uppercase italic tracking-wider shadow-lg shadow-red-900/20">
                      رفض نهائي
                    </button>
                  </div>}
              </div>
