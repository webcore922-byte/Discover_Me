...
const SUGGESTED_TAGS = ["سرعة انفجارية", "مراوغ ماهر", "قناص أهداف", "صانع ألعاب ذكي", "ارتقاء عالي", "دقة تمرير", "قوة بدنية", "قطع كرات", "تسديدات بعيدة"];
const CheckVedio = () => {
  const {
    id
  } = useParams();
  ...
  return <div className="min-h-screen bg-[var(--color-bg-main)] text-[var(--color-text-gray)] p-4 md:p-10 font-sans">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-[var(--color-bg-card)] rounded-[2.5rem] p-8 border border-[var(--color-border)]/30 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden shadow-[var(--card-shadow)]">
          ...
          <div className="flex-1 text-center md:text-right z-10">
            ...
            {player.status === 'rejected' && player.rejectionReason && <p className="text-red-500 text-sm mt-2 font-medium bg-red-500/10 inline-block px-3 py-1 rounded-lg border border-red-500/20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 bg-[var(--color-bg-card)] rounded-[2.5rem] p-8 md:p-10 border border-[var(--color-border)]/20 shadow-[var(--card-shadow)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 ">
              <SkillSlider label="السرعة (Pace)" value={skills.pace} onChange={v => setSkills({
              ...skills,
              pace: v
            })} />
              <SkillSlider label="التسديد (Shooting)" value={skills.shooting} onChange={v => setSkills({
              ...skills,
              shooting: v
            })} />
              <SkillSlider label="التمرير (Passing)" value={skills.passing} onChange={v => setSkills({
              ...skills,
              passing: v
            })} />
              <SkillSlider label="المراوغة (Dribbling)" value={skills.dribbling} onChange={v => setSkills({
              ...skills,
              dribbling: v
            })} />
              <SkillSlider label="الدفاع (Defending)" value={skills.defending} onChange={v => setSkills({
              ...skills,
              defending: v
            })} />
              <SkillSlider label="اللياقة (Physical)" value={skills.physical} onChange={v => setSkills({
              ...skills,
              physical: v
            })} />
            </div>
            ...
            <div className="mt-12 pt-8 border-t border-[var(--color-border)]/20">
              ...
              <div className="flex flex-wrap gap-3">
                {SUGGESTED_TAGS.map(tag => <button key={tag} onClick={() => toggleTag(tag)} className={`px-4 py-2 rounded-xl text-[11px]  font-bold transition-all border ${selectedTags.includes(tag) ? 'bg-[var(--color-gold-main)] text-black border-transparent scale-105' : 'bg-[var(--color-bg-main)]/30 text-[var(--color-text-gray)] border-[var(--color-border)]/30 hover:border-[var(--color-gold-main)]/50'}`}>
                    {tag}
                  </button>)}
              </div>
            </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-[var(--color-bg-card)] rounded-[2.5rem] p-8 border border-[var(--color-border)]/20 h-full flex flex-col justify-between shadow-[var(--card-shadow)]">
              <div>
                ...
                <button onClick={handleOpenVideo} className="w-full py-5 bg-[var(--color-bg-main)]/30 border border-[var(--color-border)]/30 rounded-2xl text-[var(--color-gold-main)] font-black hover:bg-[var(--color-gold-main)] hover:text-black transition-all flex items-center justify-center gap-3 group">
              ...
              <div className="space-y-4 mt-12 pt-6 border-t border-[var(--color-border)]/30">
                ...
                <button onClick={() => handleDecision('approved')} className="w-full bg-[#16a34a] text-white font-black py-5 rounded-2xl hover:bg-green-700 hover:scale-[1.01] active:scale-95 transition-all text-lg uppercase italic shadow-lg shadow-green-950/20">
                  قبول اللاعب ✅
                </button>
                
                <button onClick={() => handleDecision('rejected')} className="w-full bg-transparent border-2 border-red-500/40 text-red-500 font-black py-5 rounded-2xl hover:bg-red-600 hover:text-white transition-all text-lg uppercase">
...
const SkillSlider = ({
  ...
  onChange
    ...
    <div className="relative h-2 flex items-center">
        <input type="range" min="1" max="10" step="0.5" value={value} onChange={e => onChange(parseFloat(e.target.value))} className="w-full h-1.5 bg-[var(--color-border)]/20 rounded-lg appearance-none
