...
const NewsAndUpdates = () => {
  ...
  if (loading) {
    return <div className="min-h-screen dark:bg-[#0e1011] bg:[var(--color-bg-main)] flex items-center justify-center text-[#D4AF37] font-bold">
        جاري تحميل آخر الأخبار والتحديثات...
      </div>;
  }
  return <div className="min-h-screen dark:bg-[#0e1011] bg:[var(--color-bg-main)] dark:text-[var(--color-text-white)] text:[var(--color-text-gray)] py-12 px-4 md:px-8 lg:px-16 font-sans relative overflow-hidden bg-cover bg-center bg-no-repeat bg-fixed" style={{
      ...
      <div className="max-w-[1200px] mx-auto space-y-12 relative z-10">
        ...
        <div className="text-center space-y-2 max-w-2xl mx-auto pt-4">
          <div className="flex items-center justify-center gap-2">
            ...
            <h1 className="text-3xl md:text-5xl font-black tracking-wide dark:text-[var(--color-text-white)] drop-shadow-md">
              الأخبار والتحديثات
            </h1>
            <span className="text-[#D4AF37] text-xl">✨</span>
          </div>
        ...
        {}
        {newsList.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {newsList.map(news => <div key={news.id} className="bg-[#16191b]/80 border border-[#D4AF37]/20 rounded-2xl overflow-hidden flex flex-col justify-between group hover:border-[#D4AF37]/60 transition-all duration-300 shadow-xl backdrop-blur-md relative">
                <div>
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    <h3 className="font-bold text-base text-[var(--color-text-white)] group-hover:text-[#D4AF37] transition-colors line-clamp-2">
                      {news.title}
                    </h3>
                ...
                <div className="p-5 pt-0">
                  <button type="button" onClick={() => navigate(`/news/${news.id}`)} className="w-full py-2.5 px-4 rounded-xl font-extrabold text-black text-xs transition-all duration-300 relative z-20 cursor-pointer overflow-hidden flex items-center justify-center gap-1.5 hover:scale-[1.02] hover:shadow-[0_4px_12px_rgba(212,175,55,0.2)]" style={{
              background: 'linear-gradient(to right, #D4AF37, #bfa032)'
            }}>
          ...
          </div> : <div className="text-center py-20 dark:text-gray-500 text-sm font-light">
