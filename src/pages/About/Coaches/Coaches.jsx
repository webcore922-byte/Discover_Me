...
const ScoutPipeline = () => {
  ...
  return <div className="mt-24 w-full">
      ...
      <div className="relative flex justify-between items-center max-w-5xl mx-auto px-4">
        ...
        {steps.map((step, i) => <div key={i} className="relative z-10 flex flex-col items-center group">
...
const CoachesContent = () => {
  ...
  const showErrorAlert = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) window.location.reload();else if (result.dismiss === Swal.DismissReason.cancel) window.history.back();
    });
  };
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [tRes, cRes, consRes] = await Promise.all([fetch(`${import.meta.env.VITE_API_URL}/field-testers`), fetch(`${import.meta.env.VITE_API_URL}/coaches`), fetch(`${import.meta.env.VITE_API_URL}/consultants`)]);
  ...
  if (loading) return <div className="min-h-screen bg-[var(--color-bg-main)] flex flex-col items-center justify-center">
  ...
  return <div className="min-h-screen bg-[var(--color-bg-main)] p-6 md:p-12 font-sans" dir="rtl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <section className="flex flex-col gap-6">
          ...
          {scoutData.consultants.map(c => <div key={c.id} className="glass-card p-5 rounded-2xl flex items-center gap-4 hover-gold-card card-shine">
          ...
          <h2 className="mt-6 text-[var(--color-gold-main)] font-black text-sm uppercase tracking-widest border-r-4 border-[var(--color-gold-main)] pr-4">المدربون</h2>
          {scoutData.coaches.map(coach => <div key={coach.id} className="glass-card p-5 rounded-2xl flex items-center gap-4 hover-gold-card card-shine">
              <img src={coach.image} alt={coach.name} className="w-14 h-14 rounded-full border border-[var(--color-border)]" />
              <div>
                <p className="font-bold text-[var(--color-text-white)]">{coach.name}</p>
                <p className="dark:text-[var(--color-text-gray)] text-[10px] font-bold">{coach.info}</p>
              </div>
            </div>)}
        </section>
        ...
        <section className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {scoutData.field_testers.map(f => <div key={f.id} className="glass-card p-6 rounded-[2rem] hover-gold-card card-shine border-[var(--color-border)]/10">
