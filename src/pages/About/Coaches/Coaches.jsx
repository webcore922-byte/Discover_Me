...
const CoachesContent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!techRes.ok || !coachesRes.ok || !consRes.ok) {
          throw new Error("تأكد من تشغيل json-server على بورت 5000");
        }
  ...
  if (loading) return <div className="min-h-screen bg-[var(--color-bg-main)] flex items-center justify-center text-[var(--color-gold-main)]">جاري تحميل المدربين...</div>;
  if (error) return <div className="min-h-screen bg-[var(--color-bg-main)] flex items-center justify-center text-red-500 font-bold p-10 text-center" dir="rtl">حدث خطأ: {error} ⚠️</div>;
...
const TimelinePoint = ({ label, icon, active, isGold }) => (
  <div className="flex flex-col items-center gap-3 transition-transform hover:scale-110">
    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-2xl border-2 transition-all duration-300 ${
      isGold ? 'bg-[var(--color-gold-main)] border-[var(--color-gold-main)] text-black shadow-[0_0_15px_#eab308]' : 
      active ? 'bg-transparent border-[var(--color-gold-main)] text-[var(--color-gold-main)]' : 
