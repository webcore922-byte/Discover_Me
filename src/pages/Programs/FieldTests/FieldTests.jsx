...
const FieldTests = () => {
  ...
  return <div className="min-h-screen font-['Tajawal',_sans-serif] relative overflow-hidden flex flex-col" style={{
    backgroundColor: 'var(--color-bg-main)',
    color: 'var(--color-text-white)'
  }}>
            <div className="absolute inset-0 z-0 pointer-events-none dark:bg-[url('/bg_Field_tests.jpeg')]
                 bg-[url('/bg_Field_tests_Light.png')] bg-cover bg-center bg-no-repeat filter contrast-[1.15] brightness-[1.05] saturate-[1.2]"></div>
            ...
            <main className="relative z-10 flex-1 flex items-center max-w-[1500px] w-full mx-auto px-8 pb-12">
                <div className="relative z-20 w-full lg:w-[420px] flex flex-col gap-6 ms-auto lg:ms-0 mt-8 lg:mt-0">
                    ...
                    {isStaticUserCard && <div className="glass-card hover-gold-card card-shine rounded-[2rem] p-4 shadow-2xl relative z-30">
                            ...
                            <div className="px-3 pb-2 text-[15px] space-y-3">
                                <h3 className="font-black text-lg text-center" style={{
              color: 'var(--color-gold-main)'
            }}>انضم إلى محاربي الملعب!</h3>
                                <p className="text-xs text-center leading-relaxed mb-2" style={{
              color: 'var(--color-text-gray)'
                ...
                state: {
                  openUpgradeForm: true
                }
              })} className="flex-1 text-[#1A1D1E] py-2.5 rounded-xl text-xs font-black shadow-lg hover:opacity-90 transition-opacity cursor-pointer relative z-40" style={{
                background: 'var(--gold-gradient)'
                    ...
                    {isRejectedPlayer && <div className="glass-card hover-gold-card card-shine rounded-[2rem] p-5 shadow-2xl text-center relative z-30">
            ...
            color: 'var(--color-text-gray)'
                            ...
                            <button onClick={handleReSubmitTest} disabled={reSubmitting} className="w-full text-black font-black py-3 rounded-xl text-sm hover:opacity-90 transition-opacity disabled:opacity-50 cursor-pointer relative z-40" style={{
            background: 'var(--gold-gradient)'
          }}>
                                {reSubmitting ? 'جاري معالجة الطلب...' : 'إعادة الاختبار 🔄'}
                            </button>
                        </div>}
                    {isrejected && <div className="glass-card hover-gold-card card-shine rounded-[2rem] p-5 shadow-2xl text-center relative z-30">
                          <div className="rounded-[1.5rem] overflow-hidden mb-4 h-[180px]">
                                <img src={actionImg} alt="Action" className="w-full h-full object-cover" />
                            </div>
                            <div className="text-sm mb-5 leading-relaxed" style={{
            color: 'var(--color-text-gray)'
          }}>
                    ...
                    {isPendingPlayer && <div className="glass-card  hover-gold-card card-shine rounded-[2rem] p-5 shadow-2xl text-center relative z-30">
                          <div className="rounded-[1.5rem] overflow-hidden mb-4 h-[180px]">
                                <img src={actionImg} alt="Action" className="w-full h-full object-cover" />
                            </div>
                            <div className="text-sm mb-5 leading-relaxed" style={{
            color: 'var(--color-text-gray)'
                    ...
                    {isApprovedPlayer && <div className="glass-card hover-gold-card card-shine rounded-[2rem] p-5 shadow-2xl relative z-30">
                            <h3 className="font-black mb-4 text-xl" style={{
            color: 'var(--color-gold-main)'
          }}>
                                ركن اللاعب المقبول
                            </h3>
                            ...
                            <div className="flex justify-between items-start mb-5">
                                <div className="space-y-1">
                                    <h4 className="font-black text-[var(--color-text-white)] text-xl">{playerData.name}</h4>
                                    <p className="text-[13px] max-w-[200px] leading-tight" style={{
                color: 'var(--color-text-gray)'
              }}>
                                        {playerData.fieldTest?.date ? 'تهانينا! لقد تم قبولك للاختبارات. هذا هو موعدك الخاص:' : 'سوف يتم تحديد موعد اختبارك الميداني قريباً من قبل الإدارة.'}
                                    </p>
                                </div>
                                <div className="w-[85px] h-[85px] rounded-2xl p-1 shrink-0 flex items-center justify-center" style={{
              backgroundColor: 'var(--color-bg-card)',
              border: '1px solid rgba(166, 137, 70, 0.5)'
            }}>
                                    <img src={playerData.image} alt="Profile" className="w-full h-full rounded-xl object-cover" />
                                </div>
                            </div>

                            {playerData.fieldTest && playerData.fieldTest.date && playerData.fieldTest.location ? <div className="rounded-2xl p-4 border border-white/5 space-y-3" style={{
            backgroundColor: 'var(--color-bg-card)'
          }}>
                                    <div className="flex justify-between items-start">
                                        <p className="text-[13px] font-bold text-[var(--color-text-white)] leading-relaxed">
                  ...
                  color: 'var(--color-text-gray)'
                }}>حالة حضور الاختبار:</span>
                                            {playerData.fieldTest.isDone ? <span className="text-xs font-bold text-green-400 bg-green-500/10 px-2 py-0.5 rounded">تم الاختبار ✅</span> : <span className="text-xs font-bold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded animate-pulse">لم يتم بعد ⏳</span>}
                                        </div>

                                        <div className="flex justify-between items-center">
                                            <span className="text-[11px]" style={{
                  color: 'var(--color-text-gray)'
                }}>الكشاف: <span className="text-[var(--color-text-white)]">{playerData.fieldTest.coachName || 'كابتن محمد سمير'}</span></span>
                                            <div className="w-7 h-7 rounded-full overflow-hidden">
                                                <img src={coachImg} alt="Scout" className="w-full h-full object-cover" />
                                            </div>
                                        </div>
                                    </div>
                                </div> : <div className="rounded-2xl p-4 border border-white/5 text-center" style={{
            backgroundColor: 'var(--color-bg-card)'
          }}>
                                    <p className="font-bold text-sm animate-pulse" style={{
              color: 'var(--color-gold-main)'
            }}>
