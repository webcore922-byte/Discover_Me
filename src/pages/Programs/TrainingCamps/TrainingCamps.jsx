...
const TrainingCamps = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (playerRole && user?.email) {
          const regsRes = await fetch(`${apiURL}/camp-registrations`);
          if (regsRes.ok) {
            const allRegs = await regsRes.json();
            const mine = allRegs.filter(r => r.playerEmail?.toLowerCase() === user.email?.toLowerCase());
  ...
  const getMyReg = campId => myRegistrations.find(r => (r.camp?.id || r.camp) === campId) || null;
  const StatusBadge = ({
    status
  }) => {
    const map = {
    ...
    return <div className={`w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border text-xs font-bold ${s.bg}`}>
        {s.icon}
        {s.label}
      </div>;
  };
  const handleRegister = async camp => {
    ...
    try {
      ...
      const response = await fetch(`${apiURL}/camp-registrations`, {
        method: 'POST',
        headers: {
  ...
  const handleReapply = async camp => {
    ...
    if (!result.isConfirmed) return;
    try {
      await fetch(`${apiURL}/camp-registrations/${existing.id}`, {
        method: 'DELETE'
      });
      ...
      if (!response.ok) throw new Error('فشل الإرسال');
      setMyRegistrations(prev => [...prev.filter(r => r.id !== existing.id), newRegistration]);
      Swal.fire({
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-cover bg-center bg-no-repeat relative" style={{
      backgroundImage: `url('bg_prizes_and_competitions.jpeg')`
    }}>
        <div className="absolute inset-0 bg-black/80 pointer-events-none" />
        <div className="relative z-10 animate-spin-slow rounded-full h-16 w-16 border-t-2 border-b-2 border-[#d4af37]"></div>
      </div>;
  }
  return <section dir="rtl" className="min-h-screen text-[var(--color-text-white)] py-16 px-4 sm:px-6 lg:px-8 relative bg-cover bg-center bg-no-repeat bg-fixed overflow-hidden
      ...
      <div className="max-w-7xl mx-auto mb-16 text-center relative z-10">
        <div className="inline-block relative">
          ...
          <h2 className="text-3xl md:text-5xl font-extrabold mb-3 tracking-wide dark:text-[var(--color-text-white)] text-[--color-gold-main]">
            المعسكرات التدريبية
          </h2>
          <span className="absolute -bottom-2 -left-8 text-xl text-[#d4af37] opacity-70">✨</span>
        </div>
        <p className="mt-2 text-sm md:text-base max-w-md mx-auto" style={{
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10 mb-16">
        ...
        return <div key={camp.id} className="glass-card hover-gold-card card-shine rounded-2xl p-5 flex flex-col justify-between transition-all duration-300 relative group">
              <div>
                {}
                <div className="w-full h-48 rounded-xl overflow-hidden mb-5 relative group">
                  <img src={camp.image} alt={camp.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onError={e => {
                e.target.src = "https://images.unsplash.com/photo-1508098682722-e99c43a406b2?q=80&w=500";
              }} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent pointer-events-none" />

                  {}
                  {myReg?.status === 'accepted' && <div className="absolute top-3 left-3 bg-emerald-500 text-white text-[10px] font-black px-2.5 py-1 rounded-lg flex items-center gap-1 shadow-lg">
                      <CheckCircle className="w-3 h-3" /> مقبول
                    </div>}
                  {myReg?.status === 'rejected' && <div className="absolute top-3 left-3 bg-red-500 text-white text-[10px] font-black px-2.5 py-1 rounded-lg flex items-center gap-1 shadow-lg">
                      <XCircle className="w-3 h-3" /> مرفوض
                    </div>}
                  {myReg?.status === 'pending' && <div className="absolute top-3 left-3 bg-yellow-500/90 text-black text-[10px] font-black px-2.5 py-1 rounded-lg flex items-center gap-1 shadow-lg">
                ...
                <div className="text-center mb-4">
                  <h3 className="text-xl font-bold mb-1 text-[var(--color-text-white)]">{camp.title}</h3>
              ...
              <div className="space-y-3 relative z-20">
                ...
                {playerRole && <>
                    {}
                    {!myReg && <button type="button" onClick={() => handleRegister(camp)} className="w-full py-3 px-4 rounded-xl font-extrabold text-black text-sm tracking-wide transition-all duration-300 relative z-20 cursor-pointer overflow-hidden group/btn block hover:scale-[1.02] hover:shadow-[0_0_20px_rgba(212,175,55,0.4)]" style={{
                background: 'linear-gradient(to right, var(--color-btn-start), var(--color-btn-end))'
                    ...
                    {}
                    {myReg && <div className="space-y-2">
                        <StatusBadge status={myReg.status} />

                        {myReg.status === 'rejected' && <button type="button" onClick={() => handleReapply(camp)} className="w-full py-2.5 px-4 rounded-xl font-bold text-sm tracking-wide transition-all duration-300 cursor-pointer border border-[#d4af37]/40 text-[#d4af37] bg-[#d4af37]/10 hover:bg-[#d4af37]/20 hover:scale-[1.02] hover:shadow-[0_0_15px_rgba(212,175,55,0.2)]">
                            🔄 إعادة التقديم
                          </button>}
                      </div>}
                  </>}
                ...
                <div className="pt-2 border-t border-gray-800/60">
                  <div className="flex items-center justify-between bg-black/20 p-2 rounded-xl border border-gray-800/40">
                    ...
                    color: 'var(--color-gold-main)'
                      ...
                      <p className="text-xs font-bold text-[var(--color-text-white)]">{camp.coach?.name}</p>
                    </div>
                    <img src={camp.coach?.image} alt={camp.coach?.name} className="w-9 h-9 rounded-full object-cover border border-[#a68946]" style={{
