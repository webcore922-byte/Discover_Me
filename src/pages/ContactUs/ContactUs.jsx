...
const PROBLEM_TYPES = [
  ...
  { id: "store", label: "مشكلة في المتجر" },
];
...
const ContactUs = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    subject: "",
    message: "",
    type: location.state?.type || "general", // ← هنا بيقرأ الـ state
  });
          ...
          <div className="lg:col-span-8">
            <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] p-8 md:p-10 rounded-2xl">
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                {serverError && (
                  <div className="flex items-center gap-3 p-4 bg-red-500/10 border border-red-500/50 rounded-xl text-red-400 text-sm">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-4 bg-[var(--color-gold-main)] text-black font-black text-base rounded-xl hover:opacity-90 transition flex items-center justify-center gap-3 disabled:opacity-50"
                >
                  {loading ? (
                    <><FaSpinner className="animate-spin" /> جاري الإرسال...</>
                  ) : (
                    <><span>إرسال المشكلة</span><Icons.Send /></>
                  )}
                </button>
              </form>
            </div>
