...
const SuccessStoriesContent = () => {
  ...
  const stories = [{
    ...
    description: "محمد صالح - رحلة الفخر والعطاء. إنه نموذج للاعب الذي جمع بين الموهبة والأخلاق العالية، وأصبح مصدر فخر للأمة العربية كلها.",
    ...
    description: "ليونيل ميسي - أسطورة الإرادة منذ الصغر. رغم التحديات الصحية التي واجهها، لم يستسلم أبداً وأثبت أن الإرادة القوية تصنع المستحيل. قصته تلهم الملايين حول العالم.",
    ...
    description: "كريستيانو رونالدو - قمة التحدي والنجاح المستدام. بدأ رحلته من الصفر ووصل إلى القمة بإصراره . إن قصته ليست فقط للإعجاب، بل لتكون مصدر إلهام لكل من يسعى لتحقيق حلمه.",
    image: "/success-stories-cr.webp"
  }];
  return <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-cover bg-center bg-[url('../bg_success_stories-light.png')] 
    dark:bg-[url('../bg_success_stories.jpeg')]">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <Typography variant="h1" className="text-4xl md:text-5xl font-bold text-[var(--color-gold-main)] mb-4">
            قصص النجاح
          </Typography>
          <div className="w-24 h-1 bg-[var(--color-gold-main)] mx-auto mb-6 rounded-full"></div>
          <Typography variant="lead" className="dark:text-[var(--color-text-white)] max-w-3xl mx-auto text-sm text-[var(--color-text-main)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {stories.map(story => <Card key={story.id} className="relative bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl shadow-2xl overflow-hidden group">
              <CardHeader shadow={false} floated={false} className="relative h-48 overflow-hidden rounded-none">
                <img src={story.image} alt={story.name} className="w-full object-cover  " />
              </CardHeader>

              <CardBody>
                <Typography variant="h3" className="text-2xl font-bold text-[var(--color-gold-main)] mb-2">
